...
GByteArray* suck_file( char* file )
{
	...
	if((f = fopen_read_or_die(file))==NULL) return NULL;
...
int spit_file(char* file, GByteArray* b, int free )
{
	FILE* f;

	if((f = fopen_write_or_die(file)) == NULL) return -1;
...
int read_cpabe_file(char* file, GByteArray** cph_buf, int* file_len, GByteArray** aes_buf )
{
	FILE* f;
	int i;
	int len;
	...
	*aes_buf = g_byte_array_new();
	...
	if((f = fopen_read_or_die(file)) == NULL) return -1;
